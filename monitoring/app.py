...
st.write("This dashboard connects to a DynamoDB table to visualize live prediction data.")
...
if df.empty:
    st.warning("No data found in the prediction_logs table. Please send predictions from the API.")
else:
    ...
    with col2:
        ...
        predictions_over_time = df.set_index('timestamp').resample('h').size().reset_index(name='count')
        fig_time = px.line(
